    ...
    const deleteTask = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useCallback"])(async (id)=>{
        try {
            ...
            if (response.status === 200) {
                console.log(`TaskContext: Task ID: ${id} deleted successfully from API.`);
                setTodos((prevTodos)=>{
                    const filteredTodos = prevTodos.filter((task)=>task.id !== id);
                    localStorage.setItem("tasks", JSON.stringify(filteredTodos));
                    console.log(`TaskContext: Task ID: ${id} removed from local state and localStorage.`);
            ...
            console.error(`TaskContext: Error deleting task ID: ${id} from API.`, error);
        }
    }, []);
    const contextValue = {
        todos,
