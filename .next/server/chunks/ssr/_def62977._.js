...
const TaskList = (todo)=>{
    ...
    const handleCheckboxChange = async (newCompletedStatus)=>{
        ...
        if (!isPotentiallyApiTask) {
            console.log("TaskList: Task has a local ID. Skipping API update for checkbox.");
            return;
        }
        try {
            ...
            if (response.status === 200) {
                console.log("TaskList: Task updated successfully on API:", response.data);
                addOrUpdateTask(response.data);
            } else {
                console.error(`TaskList: API update failed for task ID ${todo.id}. Status: ${response.status}`, response.data);
            }
        } catch (error) {
            console.error(`TaskList: Error during API patch for task ID ${todo.id}:`, error);
        }
    };
...
const formSchema = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$index$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__$3c$locals$3e$__["default"].object({
    todo: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$index$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__$3c$locals$3e$__["default"].string().min(1, {
        ...
        try {
            if (isEditMode && initialData?.id) {
                ...
                try {
                    ...
                    if (response.status === 200) {
                        console.log("TaskForm: API update successful. Data:", response.data);
                        taskToUpdateLocally = response.data;
                    } else {
                        console.warn("TaskForm: API update failed with status", response.status, ". Updating locally.");
                    }
                } catch (apiError) {
                    console.error("TaskForm: Error during API PATCH:", apiError, ". Updating locally.");
                ...
                try {
                    ...
                    if (response.status === 201) {
                        ...
                        console.warn("TaskForm: API add failed with status", response.status, ". Saving locally.");
                        setSubmitError("Failed to add task to server. Saved locally.");
                    }
                } catch (apiError) {
                    console.error("TaskForm: Error during API POST:", apiError, ". Saving locally.");
                    setSubmitError("Failed to add task to server. Saved locally.");
                }
            }
            ...
            setSubmitError(finalError?.response?.data?.message || `An unexpected error occurred. Please try again.`);
        } finally{
            setIsSubmitting(false);
        }
    }
